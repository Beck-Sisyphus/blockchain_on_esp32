...
extern char latest_block[BLOCKSIZE];
...
static void return_bin_file( void )
{
    ...
    uint32_t read_len = 0;
    char* read_buf = malloc(BUFFSIZE);
    char* data_header = malloc(DATA_HEADER_SIZE);
...
static void return_whole_chain( void )
{
    ...
    while(1)
    {
        if(i <= 2)//latest_block->index)
        {
            esp_err_t err = read_block_on_chain(i , read_buf , BLOCKSIZE);
            if(err != ESP_OK){ESP_LOGE(TAG,"Read block %d on chain failed. error type:%d", i , err);}
            else{ESP_LOGI(TAG,"Read block %d on chain succeed!. content: %s", i , read_buf);}
        }
        else break;
    }
    chunk_end(client_fd);
