...
{ //write latest block to latest_block_partition
    if(stc == NULL){
        ESP_LOGE(TAG,"ERROR: NULL pointer in function latest_block_write!\n");
        return;
    }
    ...
    if(!latest_block_partition){
        ...
        if(err == ESP_OK){
            ESP_LOGE(TAG,"esp_partition_write genesis_bolck to latest_block succeed!\n");
        }else{
            ESP_LOGE(TAG,"esp_partition_write genesis_bolck to latest_block failed! error = %d\n" , err);
        }
    }
    return;
}
...
{ //read latest block from latest_block_partition
    ...
    if(!latest_block_partition){
        ...
        if(err == ESP_OK){
            ESP_LOGE(TAG,"esp_partition_read genesis_bolck from latest_block succeed!\n");
        }else{
            ESP_LOGE(TAG,"esp_partition_read genesis_bolck from latest_block failed! error = %d\n" , err);
        }
    }
    return l_block;
}
...
void write_block_on_chain(uint8_t index , const void* src )
{
    ...
    if(!chain_partition){
        ...
        if(err == ESP_OK){
            ESP_LOGE(TAG,"Write %d th block on chain succeed!\n", index);
        }else{
            ESP_LOGE(TAG,"Write %d th block on chain failed! error = %d\n" , index , err);
        }
    }
    return;
}
